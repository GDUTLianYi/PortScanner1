...
BOOL CPortScannerDlg::OnInitDialog()
{
	...
	HWND h1 = ::GetDlgItem(m_hWnd, IDC_LIST_PORT);
	m_list.Attach(h1);
	//pList = (CListCtrl*)GetDlgItem(IDC_LIST_PORT);
	m_list.SetExtendedStyle(LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES);
	m_list.InsertColumn(0, "IP地址", 0, 180);
	m_list.InsertColumn(1, "端口地址", 0, 120);

	h1 = ::GetDlgItem(m_hWnd, IDC_IPADDRESS); m_IP1.SubclassWindow(h1);
	h1 = ::GetDlgItem(m_hWnd, IDC_IPADDRESS1); m_IP2.SubclassWindow(h1);
	//CComboBox* pComb = (CComboBox*)GetDlgItem(IDC_COMBO1);
	h1 = ::GetDlgItem(m_hWnd, IDC_COMBO1); m_CombTime.SubclassWindow(h1);
	m_CombTime.AddString("500");
	m_CombTime.AddString("800");
	m_CombTime.AddString("1000");
	m_CombTime.AddString("1200");
	m_CombTime.AddString("1400");
	m_CombTime.AddString("5000");
	m_CombTime.SetCurSel(3);

	
	//CComboBox* pCombThread = (CComboBox*)GetDlgItem(IDC_COMBO_THREAD);
	h1 = ::GetDlgItem(m_hWnd, IDC_COMBO_THREAD); m_CombThread.SubclassWindow(h1);
	m_CombThread.AddString("32");
	m_CombThread.AddString("64");
	m_CombThread.AddString("128");
	m_CombThread.AddString("256");
	m_CombThread.AddString("512");
	m_CombThread.AddString("1024");
	m_CombThread.AddString("2048");
	m_CombThread.AddString("4096");
	m_CombThread.AddString("8192");
	
	m_CombThread.SetCurSel(3);

	h1 = ::GetDlgItem(m_hWnd, IDC_PROGRESS1); m_Progress.SubclassWindow(h1);
...
DWORD WINAPI ThreadFunc(LPVOID pParam) {
	...
	cClient = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP); //创建一个客户端socket
...
void CPortScannerDlg::OnBnClickedButton2()
{
	...
	m_IP1.GetAddress(f[0], f[1], f[2], f[3]);
	m_IP2.GetAddress(g[0], g[1], g[2], g[3]);
	...
	m_CombTime.GetWindowTextA(str);
	int usetime = _ttoi(str);
	m_CombThread.GetWindowTextA(str);
	int col = _ttoi(str);
			...
			for (int f2 = f[2]; f2 <= g[2]; f2++) {
				...
				for (int f3 = f[3]; f3 <= g[3]; f3++) {
					...
					if (pack[f3].port != 0) {
						...
						m_list.InsertItem(ncount, pack[f3].IP);
						CString sport; sport.Format("%d", pack[f3].port);
						m_list.SetItemText(ncount, 1, sport);
					}
					CloseHandle(hThrds[f3]);
				}
...
void CPortScannerDlg::OnBnClickedButton1()
{
	...
	m_Progress.SetRange(0, range *max(1, 1024 / col));
	m_Progress.SetPos(0);
	CString res;
	int cnt = 0;
	for (int f0 = f[0]; f0 <= g[0]; f0++) {
		for (int f1 = f[1]; f1 <= g[1]; f1++) {
			for (int f2 = f[2]; f2 <= g[2]; f2++) {
				...
				for (int f3 = f[3]; f3 <= g[3]; f3++) {
					...
					//szIP = "10.21.32.111";
					for (int j = 0; j < max(1, 1024 / col); j++) {
						for (int p = 0; p <= min(1023,col); p++) {
						...
						for (int p = 0; p < min(1023, col); p++) {
							...
							if (pack[p].port != 0) {
								...
								//	res.Append(tmp);
								int ncount = m_list.GetItemCount();
								m_list.InsertItem(ncount, pack[p].IP);
								CString sport; sport.Format("%d", pack[p].port);
								m_list.SetItemText(ncount, 1, sport);
							}
							CloseHandle(hThrds[p]);
						}
						m_Progress.SetPos(++cnt);
						UpdateWindow();
					}
				}
			}
...
void CPortScannerDlg::OnBnClickedButton3()
{
	...
	m_Progress.SetRange(0, range * 65536 / col);
	m_Progress.SetPos(0);
	CString res;
...
{
	// TODO: 在此添加控件通知处理程序代码
	CFileDialog fdlg(false,"txt",NULL);
...
void CPortScannerDlg::OnDestroy()
{
	CDialogEx::OnDestroy();
	m_list.Detach();
	m_IP1.UnsubclassWindow();
	m_IP2.UnsubclassWindow();
	m_CombTime.UnsubclassWindow();
	m_CombThread.UnsubclassWindow();
	m_Progress.UnsubclassWindow();
	// TODO: 在此处添加消息处理程序代码
}
